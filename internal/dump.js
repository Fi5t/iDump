...
function Runtime() {
    ...
    function findIvar(name) {
      ...
      if (impl === null) {
        ...
        impl = {
          ...
          set(value) {
            write2(address, toNative.call(instance, value));
          }
        };
        entry[1] = impl;
      }
  ...
  function enumerateLoadedClasses(...args) {
    ...
    for (let i = 0; i !== numClasses; i++) {
      ...
      let name = null;
      let modulePath = modules2.findPath(rawName);
      ...
      if (possiblySwift) {
        ...
        if (probablySwift) {
          ...
          modulePath = modules2.findPath(nominalTypeDescriptor);
        }
      }
      if (modulePath !== null) {
        if (name === null)
...
function getNativeFunction(name, ret, args) {
  ...
  const fn = new NativeFunction(nptr, ret, args);
...
var NSSearchPathForDirectoriesInDomains = getNativeFunction("NSSearchPathForDirectoriesInDomains", "pointer", ["int", "int", "int"]);
var wrapper_open = getNativeFunction("open", "int", ["pointer", "int", "int"]);
var read = getNativeFunction("read", "int", ["int", "pointer", "int"]);
var write = getNativeFunction("write", "int", ["int", "pointer", "int"]);
var lseek = getNativeFunction("lseek", "int64", ["int", "int64", "int"]);
var close = getNativeFunction("close", "int", ["int"]);
var remove = getNativeFunction("remove", "int", ["pointer"]);
var access = getNativeFunction("access", "int", ["pointer", "int"]);
var dlopen = getNativeFunction("dlopen", "pointer", ["pointer", "int"]);
var NS_DOCUMENT_DIRECTORY = 9;
var NS_USER_DOMAIN_MASK = 1;
function getDocumentDir() {
  const npdirs = NSSearchPathForDirectoriesInDomains(NS_DOCUMENT_DIRECTORY, NS_USER_DOMAIN_MASK, 1);
  return new frida_objc_bridge_default.Object(npdirs).objectAtIndex_(0).toString();
}
...
function swap32(value) {
  return (value & 255) << 24 | (value & 65280) << 8 | (value & 16711680) >> 8 | value >>> 24 & 255;
}
var CHUNK_SIZE = 4 * 1024 * 1024;
var chunkBuf = Memory.alloc(CHUNK_SIZE);
function openModuleFiles(mod, docDir) {
...
function copyBinary(fmodule, foldmodule, modbase) {
  const buf = chunkBuf;
  const BUFSIZE = 4096;
  ...
  if (magic == FAT_CIGAM || magic == FAT_MAGIC) {
    ...
    lseek(fmodule, 0, SEEK_SET);
    let readLen;
    while ((readLen = read(foldmodule, buf, BUFSIZE)) > 0) {
      write(fmodule, buf, readLen);
    }
  }
}
function patchEncryptionInfo(fmodule, modbase, headerSize) {
  const ncmds = getU32(modbase.add(16));
  let cmdOff = headerSize;
  let cryptidOffset = null;
  let cryptOff = 0;
  let cryptSize = 0;
  let cmd = 0;
  let cmdsize = 0;
...
function loadAllDynamicLibrary(app_path) {
  ...
  for (let i = 0, l = filenames.count(); i < l; i++) {
    ...
    if (file_name.hasSuffix_(".bundle") || file_name.hasSuffix_(".momd") || file_name.hasSuffix_(".strings") || file_name.hasSuffix_(".appex") || file_name.hasSuffix_(".app") || file_name.hasSuffix_(".lproj") || file_name.hasSuffix_(".storyboardc")) {
      continue;
    }
    if (file_name.hasSuffix_(".framework")) {
      const bundle = frida_objc_bridge_default.classes.NSBundle.bundleWithPath_(file_path);
      if (!bundle.isLoaded()) {
        bundle.load();
      }
    } else {
...
function sendFileChunked(payload, filePath) {
  ...
  for (let i = 0; i < numChunks; i++) {
    ...
    while (bytesRead < toRead) {
      const n = read(fd, chunkBuf.add(bytesRead), toRead - bytesRead);
      if (n <= 0)
        break;
      bytesRead += n;
    }
    send(Object.assign({}, payload, { size: totalSize, chunk: i, chunks: numChunks }), chunkBuf.readByteArray(bytesRead));
    recv("ack", function(_msg) {
    }).wait();
  }
  close(fd);
}
function sendAppBundleViaFrida(appPath, dumpedPaths) {
  const appBaseName = appPath.split("/").pop();
  const fm = frida_objc_bridge_default.classes.NSFileManager.defaultManager();
  const nsAppPath = frida_objc_bridge_default.classes.NSString.stringWithString_(appPath);
  const errorPtr = Memory.alloc(Process.pointerSize);
  errorPtr.writePointer(NULL);
  ...
  const sentRealPaths = /* @__PURE__ */ new Set();
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (sentRealPaths.has(realPath))
      continue;
    sentRealPaths.add(realPath);
    if (dumpedPaths.has(fullPath) || dumpedPaths.has(realPath))
      continue;
...
function handleMessage(message) {
  ...
  modules = getAllAppModules();
  const app_path = frida_objc_bridge_default.classes.NSBundle.mainBundle().bundlePath();
  if (!isUsb) {
    loadAllDynamicLibrary(app_path);
    modules = getAllAppModules();
  }
  const dumpedPaths = /* @__PURE__ */ new Set();
  for (let i = 0; i < modules.length; i++) {
